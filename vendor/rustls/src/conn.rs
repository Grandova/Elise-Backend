...
impl<Data> ConnectionCommon<Data> {
    ...
    /// but not yet processed as a full TLS record. Used by protocols like XTLS Vision when unwrapping
    /// to a direct raw TCP connection.
    pub fn take_unprocessed_transport_bytes(&mut self) -> Vec<u8> {
        let filled = self.deframer_buffer.filled();
        let bytes = filled.to_vec();
        self.deframer_buffer.discard(bytes.len());
        bytes
    }
