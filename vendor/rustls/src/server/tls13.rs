...
use super::server_conn::{ServerConnectionData, ServerEchStatus};
...
mod client_hello {
    ...
    pub(in crate::server) struct CompleteClientHelloHandling {
        ...
        pub(in crate::server) ech_inner_random: Option<Random>,
        pub(in crate::server) ech_wire_client_hello: Option<Vec<u8>>,
    }
    ...
    impl CompleteClientHelloHandling {
        ...
        pub(in crate::server) fn handle_client_hello(
            ...
            let Some(chosen_share_and_kxg) = chosen_share_and_kxg else {
                ...
                if self.ech_inner_random.is_some() {
                    let inner_msg = if let Some(wire_hs) = &self.ech_wire_client_hello {
                        Message {
                            ...
                            payload: MessagePayload::Handshake {
                                ...
                                encoded: Payload::new(wire_hs.clone()),
                            },
                        }
                    } else {
                        Message {
                            ...
                            payload: MessagePayload::handshake(HandshakeMessagePayload(
                                HandshakePayload::ClientHello(client_hello.clone()),
                            )),
                        }
                    };
                    self.transcript.add_message(&inner_msg);
                } else {
    ...
    fn emit_server_hello(
        ...
        ech_inner_random: Option<Random>,
        suite: &'static Tls13CipherSuite,
        cx: &mut ServerContext<'_>,
        ...
        // Do key exchange
        let mut key_schedule_start = key_schedule_pre_handshake.into_handshake(ckx.secret);

        let sh = if let Some(inner_random) = ech_inner_random {
            // ECH RFC draft-ietf-tls-esni-18 section 7.2:
            // 1. Calculate accept_confirmation on transcript with ServerHello where last 8 bytes of random are 0.
            let mut sh_conf_random = randoms.server;
            sh_conf_random[24..32].fill(0);
            let sh_conf = Message {
                version: ProtocolVersion::TLSv1_2,
                payload: MessagePayload::handshake(HandshakeMessagePayload(
                    HandshakePayload::ServerHello(ServerHelloPayload {
                        legacy_version: ProtocolVersion::TLSv1_2,
                        random: Random::from(sh_conf_random),
            ...
            conf_transcript.add_message(&sh_conf);
            let conf_hash = conf_transcript.current_hash();
            let conf_secret =
                key_schedule_start.server_ech_confirmation_secret(&inner_random.0, conf_hash);
            debug!("[ECH] conn={} computed accept_confirmation", cx.data.conn_id);

            // 2. Put accept_confirmation into last 8 bytes of ServerHello.random
            let mut final_random = randoms.server;
            final_random[24..32].copy_from_slice(&conf_secret);
            ...
            Message {
                ...
                payload: MessagePayload::handshake(HandshakeMessagePayload(
                    HandshakePayload::ServerHello(ServerHelloPayload {
                        legacy_version: ProtocolVersion::TLSv1_2,
                        random: Random::from(final_random),
                        session_id: *session_id,
                        cipher_suite: suite.common.suite,
                        compression_method: Compression::Null,
                        extensions,
        ...
        let key_schedule = key_schedule_start.derive_server_handshake_secrets(
            handshake_hash,
            &*config.key_log,
            &randoms.client,
            cx.common,
...
impl State<ServerConnectionData> for ExpectFinished {
        ...
        Self: 'm,
    {
        ...
        cx.data.handshake_completed = true;
        let alpn_str = cx.common.get_alpn_protocol()
            .and_then(|s| core::str::from_utf8(s).ok())
            .unwrap_or("none");
        let inner_sni_str = cx.data.inner_sni.as_deref().unwrap_or("none");
        let outer_sni_str = cx.data.outer_sni.as_deref().unwrap_or("none");
        let ech_str = match cx.data.ech_status {
            ServerEchStatus::Accepted => "accepted",
            ServerEchStatus::Rejected => "rejected",
            ServerEchStatus::NotOffered => "not_offered",
        };
        info!(
            "[TLS] conn={} TLS handshake completed version=TLSv1.3 cipher={:?} alpn={} inner_sni={} outer_sni={} ech={}",
            cx.data.conn_id,
