...
impl ExpectClientHello {
    ...
    pub(super) fn with_certified_key(
        ...
        ech_wire_client_hello: Option<Vec<u8>>,
...
impl State<ServerConnectionData> for ExpectClientHello {
        ...
        Self: 'm,
    {
        ...
        let raw_outer_ch = match &m.payload {
        ...
        if let Some(ServerNamePayload::SingleDnsName(dns_name)) = &client_hello.server_name {
            cx.data.outer_sni = Some(alloc::string::ToString::to_string(dns_name.to_lowercase_owned().as_ref()));
        }

        let (effective_client_hello, ech_inner_random, ech_wire_client_hello) =
            if self.config.ech_server_keys.is_some() {
                let (ech_ch, ech_rand, ech_wire) = crate::server::ech::process_server_ech(
                    client_hello.clone(),
                    raw_outer_ch,
                    self.config.ech_server_keys.as_ref(),
                    cx.data.conn_id,
                );
                if ech_rand.is_some() {
                    cx.data.ech_status = crate::server::ServerEchStatus::Accepted;
                    if let Some(ServerNamePayload::SingleDnsName(dns_name)) =
                        &ech_ch.server_name
                    {
                        let inner_sni_str = dns_name.to_lowercase_owned();
                        cx.data.inner_sni = Some(alloc::string::ToString::to_string(inner_sni_str.as_ref()));
                        cx.data.sni = Some(inner_sni_str);
                    }
                    crate::log::info!("[ECH] conn={} accept_confirmation_generated=true", cx.data.conn_id);
                    crate::log::info!("[ECH] conn={} accepted=true", cx.data.conn_id);
                } else if client_hello.encrypted_client_hello.is_some() {
                    cx.data.ech_status = crate::server::ServerEchStatus::Rejected;
                    crate::log::warn!("[ECH] conn={} ech=rejected", cx.data.conn_id);
                } else {
                    cx.data.ech_status = crate::server::ServerEchStatus::NotOffered;
                }
                (Some(ech_ch), ech_rand, ech_wire)
            } else {
                if client_hello.encrypted_client_hello.is_some() {
                    cx.data.ech_status = crate::server::ServerEchStatus::Rejected;
                    crate::log::warn!("[ECH] conn={} ech=rejected (no server keys configured)", cx.data.conn_id);
                } else {
                    cx.data.ech_status = crate::server::ServerEchStatus::NotOffered;
                }
                (None, None, None)
        ...
        let ch_ref = effective_client_hello.as_ref().unwrap_or(client_hello);

        let sig_schemes = if ech_inner_random.is_some() {
            ch_ref.signature_schemes.clone().unwrap_or(sig_schemes)
        } else {
            sig_schemes
        };

        self.with_certified_key(
            sig_schemes,
            ch_ref,
            &m,
            cx,
