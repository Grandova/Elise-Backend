...
pub struct ServerConfig {
    ...
    /// Optional ECH server configurations and private keys (RFC draft-18).
    pub ech_server_keys: Option<Arc<Vec<crate::server::ech::EchServerConfigAndKey>>>,
}

impl ServerConfig {
    /// Sets ECH server keys.
    pub fn set_ech_server_keys(&mut self, keys: Arc<Vec<crate::server::ech::EchServerConfigAndKey>>) {
        self.ech_server_keys = Some(keys);
    }
...
mod connection {
    ...
    use super::{
        Accepted, Accepting, EarlyDataState, ServerConfig, ServerConnectionData, ServerEchStatus,
    ...
    impl ServerConnection {
        ...
        /// Unique connection ID for tracing and correlation.
        pub fn conn_id(&self) -> u64 {
            self.inner.core.data.conn_id
        }

        /// Status of ECH negotiation for this connection.
        pub fn ech_status(&self) -> ServerEchStatus {
            self.inner.core.data.ech_status
        }

        /// Negotiated Inner SNI if ECH was accepted.
        pub fn inner_sni(&self) -> Option<&str> {
            self.inner.core.data.inner_sni.as_deref()
        }

        /// Outer SNI provided by client if any.
        pub fn outer_sni(&self) -> Option<&str> {
            self.inner.core.data.outer_sni.as_deref()
        }

        /// Whether TLS handshake has fully completed (Client Finished verified).
        pub fn is_handshake_completed(&self) -> bool {
            self.inner.core.data.handshake_completed
        }

        /// Extracts any raw bytes that were read from the underlying transport into the deframer buffer
        /// but not yet processed as a full TLS record.
        pub fn take_unprocessed_transport_bytes(&mut self) -> Vec<u8> {
            self.inner.take_unprocessed_transport_bytes()
        }
    }
...
impl Accepted {
    ...
    pub fn into_connection(
        ...
        let new = match state.with_certified_key(self.sig_schemes, ch, &self.message, &mut cx, None, None) {
            Ok(new) => new,
...
pub enum ServerEchStatus {
    /// ECH was offered and successfully accepted.
    Accepted,
    /// ECH was offered but decryption or negotiation failed.
    Rejected,
    /// ECH was not offered by the client.
    NotOffered,
}
...
pub struct ServerConnectionData {
    ...
    pub conn_id: u64,
    /// ECH status for this connection.
    pub ech_status: ServerEchStatus,
    /// Inner SNI extracted from decrypted ECH.
    pub inner_sni: Option<String>,
    /// Outer SNI provided by client in cleartext.
    pub outer_sni: Option<String>,
    /// Whether TLS handshake has completed and application traffic is active.
    pub handshake_completed: bool,
}

impl Default for ServerConnectionData {
    fn default() -> Self {
        static RUSTLS_CONN_COUNTER: core::sync::atomic::AtomicU64 = core::sync::atomic::AtomicU64::new(1001);
        Self {
            sni: None,
            received_resumption_data: None,
            resumption_data: Vec::new(),
            early_data: EarlyDataState::default(),
            conn_id: RUSTLS_CONN_COUNTER.fetch_add(1, core::sync::atomic::Ordering::Relaxed),
            ech_status: ServerEchStatus::NotOffered,
            inner_sni: None,
            outer_sni: None,
