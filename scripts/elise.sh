...
get_vlessenc_val() {
    local file="$1" sec="$2" fld="$3"
    if [[ ! -f "$file" ]]; then return 1; fi
    if command -v jq >/dev/null 2>&1; then
        jq -r ".${sec}.${fld} // empty" "$file" 2>/dev/null
    elif command -v python3 >/dev/null 2>&1; then
        python3 -c "import json, sys; d=json.load(open('$file')); print(d.get('$sec', {}).get('$fld', ''))" 2>/dev/null
    else
...
show_vlessenc_keys() {
    local file="$1"
    local x_dec x_enc m_dec m_enc
    x_dec=$(get_vlessenc_val "$file" "x25519" "decryption")
    x_enc=$(get_vlessenc_val "$file" "x25519" "encryption")
    m_dec=$(get_vlessenc_val "$file" "mlkem768" "decryption")
    m_enc=$(get_vlessenc_val "$file" "mlkem768" "encryption")

    echo -e "${BOLD}========================================${RESET}"
    echo -e "${BOLD}       VLESS Encryption 密钥配置         ${RESET}"
    echo -e "配置存储文件: ${YELLOW}$file${RESET}"
    echo -e "${BOLD}========================================${RESET}"
    echo -e "${BOLD}[方案 1: X25519 模式 (强烈推荐，兼容绝大多数客户端)]${RESET}"
    echo -e "decryption (服务端解密配置，复制填入 Xboard 面板 decryption 输入框):"
    echo -e "  ${GREEN}${x_dec}${RESET}"
    echo ""
    echo -e "encryption (客户端加密配置，复制填入 Xboard 面板 encryption 输入框):"
    echo -e "  ${BLUE}${x_enc}${RESET}"
    if [[ -n "$m_dec" && "$m_dec" != "$x_dec" ]]; then
        echo "----------------------------------------"
        echo -e "${BOLD}[方案 2: ML-KEM-768 后量子模式 (PQC 抗量子计算，需客户端完整支持)]${RESET}"
        echo -e "decryption (服务端解密配置，复制填入 Xboard 面板 decryption 输入框):"
        echo -e "  ${GREEN}${m_dec}${RESET}"
        echo ""
        echo -e "encryption (客户端加密配置，复制填入 Xboard 面板 encryption 输入框):"
        echo -e "  ${BLUE}${m_enc}${RESET}"
    fi
    echo "========================================"
    echo "* 提示: 复制上方对应方案的 decryption 填入 Xboard 面板节点的 decryption 输入框，将 encryption 填入 encryption 输入框即可开启 VLESS 加密。"
}

save_new_vlessenc_keys() {
    local file="$1"
    mkdir -p "$(dirname "$file")"
    if [[ -x "$ELISE_BIN" ]]; then
        "$ELISE_BIN" vlessenc --json > "$file"
        chmod 600 "$file" 2>/dev/null || true
        return 0
    fi
    if ! command -v openssl >/dev/null 2>&1; then
        echo -e "${RED}错误: 系统缺少 openssl 或 elise 核心程序${RESET}" >&2
        return 1
    fi
    local tmp_key priv pub
    tmp_key=$(mktemp)
    openssl genpkey -algorithm X25519 -out "$tmp_key" 2>/dev/null
    priv=$(openssl pkey -in "$tmp_key" -outform DER 2>/dev/null | tail -c 32 | base64 | tr '+/' '-_' | tr -d '=\n')
    pub=$(openssl pkey -in "$tmp_key" -pubout -outform DER 2>/dev/null | tail -c 32 | base64 | tr '+/' '-_' | tr -d '=\n')
...
cmd_vlessenc() {
    local key_file="/etc/elise/vlessenc.json"
    local action="${1:-menu}"
    case "$action" in
        gen|renew|--force|-f)
            check_root
            if save_new_vlessenc_keys "$key_file"; then
                echo -e "${GREEN}全新 VLESS Encryption 密钥已重新生成并保存！${RESET}"
                show_vlessenc_keys "$key_file"
            fi
            ;;
        show)
            if [[ -f "$key_file" ]]; then
                ...
                check_root
                save_new_vlessenc_keys "$key_file"
                show_vlessenc_keys "$key_file"
            fi
            ;;
        menu|*)
            if [[ -f "$key_file" ]]; then
                ...
                echo ""
                echo "========================================"
                echo -e "  ${BOLD}1.${RESET} 保持当前配置并返回 (默认)"
                echo -e "  ${BOLD}2.${RESET} 重新生成并覆盖保存全新密钥"
                echo "========================================"
                read -rp "请选择操作 [1-2, 默认1]: " sub_opt
                case "$sub_opt" in
                    2)
                        check_root
                        if save_new_vlessenc_keys "$key_file"; then
                            echo -e "${GREEN}全新密钥已重新生成并覆盖保存！${RESET}"
                            show_vlessenc_keys "$key_file"
                        fi
                        ;;
                    *)
...
cmd_plugin() {
    local plugin="${1:-kcptun}"
    case "$plugin" in
        kcptun|kcptun-server)
            check_root
            echo -e "${BLUE}正在自动检测架构并安装 kcptun-server ...${RESET}"
            local arch
            case "$(uname -m)" in
                x86_64|amd64) arch="amd64" ;;
                aarch64|arm64) arch="arm64" ;;
                *) echo -e "${RED}不支持的架构: $(uname -m)${RESET}" >&2; return 1 ;;
            esac
            local url="https://github.com/nebulabox/kcptun/releases/download/2025.10.17/kcptun-linux-${arch}-20251017.tar.gz"
            local fallback_url="https://github.com/dumbybumby/kcptun-archive/releases/download/20260411/kcptun-server-linux-${arch}"
            echo "下载地址: $url"
            local tmp
            tmp=$(mktemp -d)
            if curl -fL --progress-bar "$url" -o "$tmp/kcptun.tar.gz" 2>/dev/null && tar -xzf "$tmp/kcptun.tar.gz" -C "$tmp" 2>/dev/null && [[ -f "$tmp/server_linux_${arch}" ]]; then
                install -m 755 "$tmp/server_linux_${arch}" /usr/local/bin/kcptun-server
                mkdir -p /etc/elise
                install -m 755 "$tmp/server_linux_${arch}" /etc/elise/kcptun-server
                rm -rf "$tmp"
                echo -e "${GREEN}kcptun-server 安装成功！路径: /usr/local/bin/kcptun-server${RESET}"
                echo -e "${GREEN}您可以运行 'elise restart' 重启服务。${RESET}"
                return 0
            elif curl -fL --progress-bar "$fallback_url" -o "$tmp/kcptun-server" 2>/dev/null && [[ -s "$tmp/kcptun-server" ]]; then
                install -m 755 "$tmp/kcptun-server" /usr/local/bin/kcptun-server
                mkdir -p /etc/elise
                install -m 755 "$tmp/kcptun-server" /etc/elise/kcptun-server
                rm -rf "$tmp"
                echo -e "${GREEN}kcptun-server (备用镜像) 安装成功！路径: /usr/local/bin/kcptun-server${RESET}"
                echo -e "${GREEN}您可以运行 'elise restart' 重启服务。${RESET}"
                return 0
            else
                rm -rf "$tmp"
                echo -e "${RED}下载或安装失败，请检查网络连接${RESET}" >&2
                return 1
            fi
            ;;
        *)
            echo -e "${YELLOW}用法: elise plugin kcptun${RESET}"
            ;;
    esac
}
...
${BOLD}证书与 Reality / VLESS 加密:${RESET}
  elise cert                    证书管理
  ...
  elise vlessenc                查看/管理已保存的 VLESS Encryption 密钥 (/etc/elise/vlessenc.json)
  elise vlessenc gen            重新生成并覆盖保存全新 VLESS Encryption 密钥

${BOLD}更新与维护:${RESET}
  elise update                  更新到最新正式版
  elise update beta             更新到最新测试版
  elise update <version>        更新到指定版本 (如: elise update v1.0.1)
  elise plugin [kcptun]         一键安装官方第三方插件 (如 kcptun-server)
...
interactive_menu() {
    ...
    while true; do
        ...
        echo -e "  ${BOLD}11.${RESET} Reality 密钥管理 (X25519)"
        echo -e "  ${BOLD}12.${RESET} VLESS Encryption 密钥管理 (decryption/encryption)"
        echo "----------------------------------------"
        echo -e "  ${BOLD}13.${RESET} 设置开机自启"
        echo -e "  ${BOLD}14.${RESET} 取消开机自启"
        echo -e "  ${BOLD}15.${RESET} 更新 Elise（正式版）"
        echo -e "  ${BOLD}16.${RESET} 更新 Elise（测试版）"
        echo -e "  ${BOLD}17.${RESET} 卸载 Elise"
        echo -e "  ${BOLD}18.${RESET} 查看版本"
        echo "----------------------------------------"
        ...
        case "$choice" in
            ...
            11) cmd_x25519; pause ;;
            12) cmd_vlessenc; pause ;;
            13) cmd_enable; pause ;;
            14) cmd_disable; pause ;;
            15) cmd_update stable; pause ;;
            16) cmd_update beta; pause ;;
            17) cmd_uninstall ;;
            18) cmd_version; pause ;;
...
main() {
    ...
    case "$cmd" in
        ...
        reality) cmd_reality "$@" ;;
        x25519) cmd_x25519 "$@" ;;
        vlessenc|vless-enc) cmd_vlessenc "$@" ;;
