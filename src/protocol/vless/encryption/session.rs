...
impl SessionStore {
    ...
    /// Stores a new session associated with the 16-byte ticket and TTL.
    /// Employs throttled cleanup (at most once every 30s) and a hard capacity cap (16384 entries).
    pub fn insert(&self, ticket: [u8; 16], pfs_key: [u8; 64], ttl: Duration) {
        let now = Instant::now();
        let mut map = self.sessions.write();

        // 1. Throttled periodic eviction: at most once every 30 seconds
        let should_sweep = {
        ...
        // 2. Strict memory cap: if still at maximum capacity, reject insert to prevent OOM
    ...
    /// Validates ticket and checks for 0-RTT replay attacks using nfs_key.
    /// Caps nfs_keys per ticket to 256 to prevent memory bloating from replayed tickets.
