...
/// 按照 BLAKE3 规范安全计算 DERIVE_KEY_CONTEXT 域哈希（支持任意二进制上下文且无 unsafe）
fn hash_derive_key_context_safe(context: &[u8]) -> [u8; 32] {
    if context.len() <= 1024 {
        ...
        // 多分块树形折叠：VLESS 上下文最大为 ML-KEM-768 公钥 (1216 字节)，在 2048 字节以内
...
/// 使用 BLAKE3 KDF 派生 32 字节子密钥。
///
/// 严格对齐上游 Go 实现：`blake3.DeriveKey(k, string(ctx), key)`。
/// 彻底根除 `unsafe std::str::from_utf8_unchecked`，对任意二进制上下文（IV、票据、公钥等）安全可用。
pub fn derive_key(context: &[u8], key_material: &[u8]) -> [u8; 32] {
    // 步骤 1：在 DERIVE_KEY_CONTEXT 域安全派生 32 字节上下文密钥
    let context_key = hash_derive_key_context_safe(context);

    // 步骤 2：在 DERIVE_KEY_MATERIAL 域使用官方 SIMD Hasher 派生子密钥
    let mut hasher = blake3::Hasher::new_from_context_key(&context_key);
    hasher.update(key_material);
    *hasher.finalize().as_bytes()
}

/// 计算输入数据的 32 字节 BLAKE3 哈希值。
/// 对齐上游 Go：`blake3.Sum256(data)`。
...
mod tests {
    ...
    /// Golden vector generated from Xray-core Go:
    ///   ctx  = [0x00, 0x01, ..., 0x0f]  (16 bytes)
    ///   key  = "01234567890123456789012345678901" (32 bytes ASCII)
    ///   out  = blake3.DeriveKey(string(ctx), key_material)
    ///
    /// NOTE: This vector was generated using the low-level BLAKE3 derive_key API
    /// directly (context hash → material hash), matching `blake3::Hasher::new_derive_key_context`.
    #[test]
    ...
    fn test_derive_key_binary_context_null_bytes() {
        // Context with 0x00 bytes — previously would trigger UB with from_utf8_unchecked
    ...
    fn test_derive_key_binary_context_high_bytes() {
        // 0x80, 0xFF — invalid UTF-8, safe with this implementation
    ...
    fn test_derive_key_random_iv_ticket() {
        // Simulate realistic IV (16 bytes) used in VLESS encryption KDF
        let iv: [u8; 16] = [
    ...
    /// 黄金测试向量：与 Xray-core Go `blake3.DeriveKey(k, string(iv), key)` 精确对齐检验
    #[test]
    ...
    /// 多分块测试：验证 >1024 字节上下文（如 ML-KEM-768 1216 字节公钥）树形折叠哈希
    #[test]
