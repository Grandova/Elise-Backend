...
impl VlessAead {
    /// Creates a new AEAD instance deriving a 32-byte key from (ctx, key) using BLAKE3 KDF.
    /// Matches upstream Go: `NewAEAD(ctx, key, useAES)`.
    ...
    /// Increments internal nonce (or uses explicit nonce) and encrypts plaintext with additional authenticated data.
    pub fn seal(
        &mut self,
    ...
    /// Increments internal nonce (or uses explicit nonce) and decrypts ciphertext with additional authenticated data.
    pub fn open(
        &mut self,
        explicit_nonce: Option<&[u8; 12]>,
        ciphertext: &[u8],
