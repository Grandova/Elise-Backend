...
pub struct HandshakeServerConfig {
    pub nfs_private_key: Vec<u8>, // 32B X25519 or 64B/2400B ML-KEM-768
    pub nfs_public_key: Vec<u8>,  // 32B X25519 pub or 1184B ML-KEM ek
    pub xor_mode: u32,            // 0=native, 1=xorpub, 2=random
...
pub async fn perform_server_handshake(
    ...
    // 1. Read IV (16B) + Relays (32B or 1088B)
    ...
    // If xor_mode > 0, unmask relays using CTR with nfs_public_key
    if cfg.xor_mode > 0 {
        let mut ctr = Aes256Ctr::new(&cfg.nfs_public_key, iv);
        ctr.xor_keystream(relays);
    }

    // 2. Compute nfsKey (authentication)
    ...
    // 3. Read encryptedLength (18 bytes) and negotiate cipher (AES vs ChaCha)
    ...
    // Extract client's ML-KEM-768 encapsulation key and X25519 public key
    ...
    // 7. Compose and write ServerHello
    let pfs_kx_len = 1088 + 32 + 16; // 1136 bytes
    ...
    // 8. Read client padding response
