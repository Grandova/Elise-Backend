//! Hysteria v1 (Protocol Version 3) Inbound Server
//! Strictly follows the official Hysteria v1 protocol specification.
//! Uses RFC 9000 QUIC + RFC 9221 QUIC Datagrams via Quinn.
...
// ============================================================================
// Hysteria v1 Datagram Reassembly (Bounded, Memory-Capped, TTL, Anti-Poisoning)
// ============================================================================
...
pub struct Hy1Defragmenter {
    entries: Mutex<HashMap<(u32, u16), Hy1ReassemblyEntry>>, // (session_id, packet_id) -> Entry
    ...
    pub fn push_fragment(
        ...
        if let Some(entry) = entries.get_mut(&key) {
            if entry.total_count != frag_total as usize || entry.host != host || entry.port != port
            {
                ...
                // Inconsistent fragment -> drop and purge poisoned entry
...
/// Helper for graceful drain of JoinSet: cancel -> graceful join -> timeout -> abort
...
impl Inbound for Hysteria1Inbound {
    ...
    fn update_users(&self, users: Vec<User>) {
        ...
        for u in users {
            ...
            // 1. Raw password string as bytes (auth_str compatibility)
            if let Some(ref pass) = u_arc.password {
                insert_cred(pass.as_bytes().to_vec(), "password string");
                // 2. Hex-decoded bytes if password is valid hex (auth binary compatibility)
            ...
            // Also unhyphenated UUID if uuid contains '-'
        ...
        loop {
            tokio::select! {
                ...
                incoming = endpoint.accept() => {
                    ...
                    // Periodic reaping of completed connection tasks
...
// ============================================================================
// Hysteria v1 Connection Lifecycle Handler (Task JoinSet + Handshake Timeout)
// ============================================================================
...
async fn handle_hy1_connection(
    ...
    // Handshake Timeout enforcement (10 seconds)
    let handshake_res = tokio::time::timeout(Duration::from_secs(10), async {
        // Step 1: Accept the first bidirectional stream - Control Stream
        ...
        // [1..9]: SendBPS (u64 BE)
        // [9..17]: RecvBPS (u64 BE)
        // [17..19]: AuthLen (u16 BE)
        // [19..19+AuthLen]: Auth (UTF-8 string)
        ...
        let user = match user_opt {
            ...
            None => {
                ...
                // ServerHello Error response: [OK=0][SendBPS=0][RecvBPS=0][MsgLen][Message]
                let mut err_resp = Vec::new();
                err_resp.push(0x00); // OK = 0
        ...
        // ServerHello OK response: [OK=1][SendBPS][RecvBPS][MsgLen=0]
    ...
    // JoinSet for structured per-connection task management
    let mut join_set = tokio::task::JoinSet::new();
    let conn_cancel = CancellationToken::new();

    // Task 1: Control stream keepalive
    ...
    // Step 2: Manage UDP Sessions & Datagram Multiplexing
    ...
    // Step 3: Accept Proxy Streams (TCP Requests & UDP Session Allocations)
    // Requirement 6: Each stream gets an independent task immediately; eliminate Slowloris.
    loop {
        tokio::select! {
            ...
            bi_res = conn.accept_bi() => {
                ...
                // Periodic reaping of completed stream tasks
    ...
    // Clean graceful shutdown of connection: cancel -> graceful join -> timeout -> abort
    conn_cancel.cancel();
...
// ============================================================================
// Hysteria v1 Stream Entry (Request-Header Timeout + Type Validation)
// ============================================================================
...
async fn handle_hy1_stream_entry(
    ...
    // Requirement 6: 5-second request header timeout to eliminate Slowloris
    ...
    // Requirement 2: Only 0x00 (TCP) and 0x01 (UDP) allowed, reject reserved
    if req_type != 0x00 && req_type != 0x01 {
        ...
        err_resp.push(0x00); // OK = 0
        err_resp.extend_from_slice(&0u32.to_be_bytes()); // UDPSessionID = 0
    ...
    if req_type == 0x01 {
        ...
        // Requirement 7: Dedicated UDP socket for this session
        ...
        // Requirement 3: Register session BEFORE sending OK to client
...
// ============================================================================
// Hysteria v1 TCP Stream Relay (Dial First -> Send ServerResponse -> Relay)
// ============================================================================
...
async fn handle_hy1_tcp_stream(
    ...
    // 4. Send official ServerResponse OK: [OK=1][UDPSessionID=0 4B][MsgLen=0 2B] (7 bytes)
...
// ============================================================================
// Hysteria v1 Dedicated UDP Session Relay (Dedicated Socket + Full Cone NAT)
// ============================================================================
...
fn send_hy1_datagram(
    ...
    if data.len() <= chunk_size {
        ...
        if total_chunks > 255 {
            return false; // Exceeds 8-bit fragment limit
        }
        let mut all_ok = true;
...
async fn handle_hy1_udp_session(
    ...
    // Structured JoinSet for all session child tasks
    let mut session_join_set = tokio::task::JoinSet::new();
    let child_cancel = session_cancel.child_token();

    // 1. Monitor stream closure: if client closes the UDP association stream, close session
    ...
    // 2. Downlink receiver task on dedicated UDP socket
        ...
        loop {
            tokio::select! {
                ...
                recv_res = tokio::time::timeout(Duration::from_secs(60), r_socket.recv_from(&mut buf)) => {
                    ...
                    // Send datagram with real received source address
                    if send_hy1_datagram(&r_conn, session_id, pkt_id, &src_host, src_port, &buf[..n]) {
                        // Requirement 8: Traffic accounting ONLY after successful datagram send
    ...
    loop {
        tokio::select! {
            ...
            packet = packet_rx.recv() => {
                ...
                // 4. Uplink rate limiting
                ctx.rate_limiter.throttle(user_id, data.len()).await;

                // 5. Send through dedicated socket
                match socket.send_to(&data, target_addr).await {
                    Ok(sent_bytes) => {
                        // Requirement 8: Traffic accounting ONLY after successful send!
    ...
    // Requirement 4: cancel -> graceful join -> timeout -> abort
    child_cancel.cancel();
...
mod tests {
    ...
    fn test_allocate_session_id_collision_avoidance() {
        ...
        assert_eq!(sid, 2); // 1 is in map, skips to 2
    }

    #[test]
    fn test_hy1_server_response_format() {
        // TCP OK response: [OK=1][UDPSessionID=0 4B][MsgLen=0 2B] -> exactly 7 bytes
        let ok_resp = [0x01u8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
        ...
        // UDP OK response with sid=12345
        let sid: u32 = 12345;
    ...
    fn test_hy1_byte_oriented_auth_and_collision_detection() {
        ...
        let user2 = User {
            ...
            password: Some("68656c6c6f".to_string()), // hex for "hello"
            ..Default::default()
        };
        // User 3 has duplicate password colliding with user 1
        let user3 = User {
            id: 103,
