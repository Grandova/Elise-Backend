...
pub async fn handle_connection(
    ...
    // 1. PROXY Protocol (v1/v2 auto-detection or strict enforcement, preserves remaining SOCKS bytes)
    let (meta, mut stream) =
    ...
    // 2. SOCKS5 Method Negotiation & RFC 1929 Auth
    let authenticated_user = negotiate_auth(&mut stream, client_ip, users, &ctx).await?;

    // 3. Limiter checks using meta.client_addr
    if !ctx
        .device_limiter
    ...
    let _conn_guard = match ctx.conn_limiter.try_acquire(authenticated_user.id) {
        Some(g) => g,
        None => {
            let _ = stream
                .write_all(&[0x05, 0x02, 0x00, 0x01, 0, 0, 0, 0, 0, 0])
    ...
    // 4. SOCKS5 Request parsing (RFC 1928 Section 4)
