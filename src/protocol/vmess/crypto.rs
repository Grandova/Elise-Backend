...
/// CRC32-IEEE implementation (table-based, zero extra dependencies)
...
// ---------------------------------------------------------------------------
// Official VMess AEAD KDF (Nested HMAC-SHA256 constructor)
// ---------------------------------------------------------------------------
...
/// Official V2Fly/sing-box VMess AEAD KDF
...
/// Derive command key from UUID: MD5(UUID + "c48619fe-8f02-49e0-b9e9-edf763e17e21")
...
/// Generate ChaCha20-Poly1305 key: MD5(key) || MD5(MD5(key))
...
// ---------------------------------------------------------------------------
// Zero-dependency pure Rust Shake128 for VMess Chunk Masking
// ---------------------------------------------------------------------------
...
const SHAKE128_RATE: usize = 168; // 1344 bits = 168 bytes
...
impl Shake128 {
    pub fn new(nonce: &[u8]) -> Self {
        ...
        // Domain separation padding for Shake128: 0x1f ... 0x80
        s.absorb_byte(0x1f);
...
impl AuthIdReplayFilter {
    ...
    /// Returns true if the AuthID is accepted (not replayed), false if duplicate
    pub fn check_and_insert(&self, auth_id: &[u8; 16], now_sec: i64) -> bool {
        let mut map = self.records.lock().unwrap();
        // Prune expired records older than 120s
...
// ---------------------------------------------------------------------------
// VMess User Keys & AuthID Validator
// ---------------------------------------------------------------------------
...
impl VmessUserKeys {
    ...
    /// Try to validate incoming 16-byte AuthID with timestamp and anti-replay
    pub fn validate_auth_id(&self, auth_id: &[u8; 16], now_sec: i64) -> bool {
        ...
        // Clock drift tolerance: +/- 120 seconds
...
/// Decrypt VMess AEAD request header length (2 bytes length encrypted in 18 bytes block)
...
/// Decrypt VMess AEAD request header
...
/// Create VMess AEAD response header (38 bytes: 18B len + 20B payload)
pub fn create_vmess_response_header(
    ...
    // 1. Length block (2 bytes plain = 4)
    ...
    // 2. Payload block (4 bytes plain = [response_header_byte, option, 0, 0])
...
/// VMess Chunk Decrypter (Client -> Outbound)
...
impl VmessChunkDecrypter {
    ...
    /// Read and decrypt length. Returns total bytes in chunk payload to read from network.
    /// Returns Ok(0) if client signaled EOF chunk (0-byte payload).
        ...
        match &mut self.len_codec {
            ChunkLengthCodec::Authenticated {
                ...
                *count = count.wrapping_add(1);
                ...
                if total_len == 16 {
                    return Ok(0); // Standard VMess EOF chunk
                }
                if total_len < 16 {
                    return Err(Error::new(
                        ErrorKind::InvalidData,
            ...
            ChunkLengthCodec::Masked {
                ...
                if total_len == 16 + padding_len {
                    return Ok(0); // Standard VMess EOF chunk with padding
                }
                if total_len < 16 + padding_len {
                    return Err(Error::new(
                        ErrorKind::InvalidData,
...
/// VMess Chunk Encrypter (Outbound -> Client)
...
mod tests {
    ...
    fn test_vmess_user_keys_and_auth_id() {
        ...
        // Encrypt with auth_id_cipher to produce auth_id
        ...
        // Anti-replay: identical AuthID in window must fail
        assert!(!user_keys.validate_auth_id(&auth_id, now_sec));
    }
    ...
    fn test_vmess_chunk_masked_roundtrip() {
        ...
        // Masked mode (sing-box / v2ray default: option 0x04)
        ...
        // 2 bytes masked length + original_data.len() + 16 bytes AEAD tag
    ...
    fn test_vmess_chunk_authenticated_roundtrip_chacha() {
        ...
        // Authenticated length with ChaCha20-Poly1305
        let mut enc = VmessChunkEncrypter::new(
            &req_key,
            &req_nonce,
    ...
    fn test_vmess_chunk_global_padding_roundtrip() {
        ...
        // 3. EOF marker chunk (empty payload)
