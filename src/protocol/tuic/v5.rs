...
// ============================================================================
// V5 UDP Reassembly / Defragmenter
// ============================================================================
...
pub struct V5Defragmenter {
    entries: Mutex<HashMap<(u16, u16), V5DefragEntry>>, // (assoc_id, packet_id) -> Entry
...
impl TuicV5Session {
    ...
    async fn handle_uni_stream(self: Arc<Self>, mut recv: quinn::RecvStream) {
        ...
        match cmd {
            CMD_AUTHENTICATE => {
                let mut auth_body = [0u8; 48]; // 16B UUID + 32B TOKEN
            ...
            CMD_PACKET => {
                ...
                let mut pkt_hdr = [0u8; 8]; // assoc_id(2), pkt_id(2), frag_total(1), frag_id(1), size(2)
                        ...
                        match mode {
                            TuicV5RelayMode::Native => {
                                ...
                                if len + header_overhead <= max_packet_size {
                                    ...
                                    packet_buf.push(1); // frag_total
                                    packet_buf.push(0); // frag_id
                            ...
                            TuicV5RelayMode::Quic => {
                                // QUIC stream handles any size directly without UDP datagram fragmentation
