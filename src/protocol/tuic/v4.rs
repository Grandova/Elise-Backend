...
impl TuicV4Session {
    ...
    async fn run_udp_association(
        ...
        let (socket, _) = match self.ctx.router.dialer().dial_udp("0.0.0.0", 0, None).await {
            Ok(s) => s,
            Err(e) => {
                warn!("TUIC V4 failed to allocate dedicated UDP socket: {:?}", e);
                return;
            }
        };
        ...
        // Inbound relay task: socket.recv_from -> package TUIC V4 Packet -> send to client
            ...
            loop {
                tokio::select! {
                    ...
                    res = socket_recv.recv_from(&mut recv_buf) => {
                        ...
                        TuicV4Address::from_socket_addr(src_addr).write_to(&mut packet_buf);
