...
/// 按照 upstream Session.writePaddedLocked 规范写出填充帧
pub async fn write_padded_frame<W: AsyncWriteExt + Unpin>(
    ...
    *packet_count += 1;
    ...
    for size in sizes {
        if size == -1 {
            // CheckMark (,c): If no remaining user data, abort further padding for this write!
        ...
        // remaining_data.len() <= target_size: pad with CMD_WASTE
...
mod tests {
    ...
    fn test_generate_payload_sizes() {
        ...
        // sizes[0] is in 400..500
        assert!(sizes[0] >= 400 && sizes[0] < 500);
        // sizes[1] is CheckMark (-1)
        assert_eq!(sizes[1], -1);
    }
}
