...
pub async fn run_anytls_stream_worker(
        ...
        .await
    {
        Ok(s) => s,
        Err(e) => {
            // [Requirement ④]: On outbound failure send error SYNACK, then close stream
            if client_version >= 2 {
    ...
    // [Requirement ④]: On outbound success send empty SYNACK
    if client_version >= 2 {
        let _ = session_tx
    ...
    // [Requirement ⑦]: Upstream task supports half-close (shutdown write on client FIN)
    let up_task = async {
        let mut up = 0u64;
        loop {
            tokio::select! {
                ...
                chunk_opt = stream_rx.recv() => {
                    match chunk_opt {
                        ...
                        None => {
                            // Client sent FIN: half-close upstream direction cleanly!
    ...
    // [Requirement ⑦]: Downstream task reads from remote target until remote EOF
    let down_task = async {
            ...
            tokio::select! {
                ...
                read_res = out_read.read(&mut buf) => {
                    let n = match read_res {
                        Ok(n) if n > 0 => n,
                        _ => break, // Remote closed (EOF) or error
                    };
        ...
        // Remote sent EOF -> notify client with CMD_FIN
        let _ = session_tx.send(make_frame(CMD_FIN, stream_id, &[])).await;
        down
    };

    // Both directions run to natural completion without tokio::select! killing the remaining half
