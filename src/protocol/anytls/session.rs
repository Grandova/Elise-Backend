...
pub async fn handle_anytls_session(
    ...
    // Channel for outgoing frames to the client
    ...
    // Dedicated writer task applying AnyTLS packet padding
    ...
    // [Requirement ⑩]: JoinSet manages all stream worker tasks within this session
    ...
    let session_res: std::io::Result<()> = async {
        loop {
            ...
            // [Requirement ⑧]: Session idle check based on bidirectional activity & active streams
            ...
            // [Requirement ⑨]: Frame read timeout to prevent slowloris
            ...
            match command {
                CMD_SETTINGS => {
                    // [Requirement ②]: Parse v, client, padding-md5 from settings
                    ...
                    // [Requirement ⑤]: MD5 comparison and CMD_UPDATE_PADDING_SCHEME
                    ...
                    // [Requirement ③]: Send cmdServerSettings ONLY when client v >= 2
                    if client_version >= 2 {
                        let _ = session_tx
                ...
                CMD_SYN => {
                    // [Requirement ②]: Enforce cmdSettings strictly precedes cmdSYN
                    ...
                    // [Requirement ⑨]: Enforce strictly monotonic increasing streamId
                    ...
                    // [Requirement ⑨]: Enforce max concurrent streams per session
                    let should_reject = {
                    ...
                    match state_opt {
                        Some(StreamState::WaitingForDestination) => {
                                ...
                                parse_socks5_addr(&payload)
                            {
                                ...
                                // [Requirement ⑥]: Detect sing-box UDP-over-TCP v2
                        ...
                        None => {
                            // Unknown or closed stream
                        }
                    }
                }
                CMD_FIN => {
                    // [Requirement ⑦]: Client closed writing (half-close).
                    // Drop stream_tx so upstream worker finishes, but downstream continues!
                    streams.write().remove(&stream_id);
                }
                CMD_HEART_REQUEST => {
                    let _ = session_tx
                ...
                _ => {
                    // Discard unknown or waste frames
                }
            }
        }
    }
