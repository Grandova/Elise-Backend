...
/// Wraps a stream with TCP HTTP camouflage if header.type is "http".
/// If header.type is "none", returns the original stream unchanged.
...
impl<S: AsyncStream> HttpCamouflageStream<S> {
    pub async fn server_handshake(mut inner: S, config: &TcpTransportConfig) -> io::Result<Self> {
        ...
        // Parse and validate HTTP request line and headers
        ...
        // Build 200 OK camouflage response header
...
fn validate_http_request(
    ...
    if let Some(cfg) = expected {
        ...
        // Check path (must match one of the URIs)
...
fn format_rfc2822_date(secs: u64) -> String {
    // Simple UTC RFC 2822 date representation: e.g. "Wed, 16 Sep 2026 03:20:00 GMT"
...
mod tests {
    ...
    async fn test_tcp_http_camouflage_success() {
        ...
        // Send valid HTTP GET request + piggybacked payload
        let req = b"GET /video/stream HTTP/1.1\r\nHost: cdn.example.com\r\nUser-Agent: Mozilla/5.0\r\n\r\nHELLO";
        client.write_all(req).await.unwrap();
        client.flush().await.unwrap();

        // Read response: first comes 200 OK header, then "WORLD"
