...
impl Router {
    ...
    pub fn match_outbound(&self, ctx: &MatchContext) -> OutboundConfig {
        ...
        // 2. Match standard structured rules
        for rule in &cfg.rules {
            ...
            // Matched rule! Find corresponding outbound
            if let Some(ob) = cfg.outbounds.iter().find(|o| o.tag == rule.outbound) {
                return ob.clone();
            }
        }

        // 3. Fallback default outbound
        cfg.outbounds
            .iter()
    ...
    fn match_toml_route(
        ...
        if dest_rules.is_empty() {
            // No destination rules specified -> matches all destinations within scope
            return true;
        }
        ...
        for rule in dest_rules {
                ...
                .or_else(|| rule.strip_prefix("ip_cidr:"))
            {
                ...
                // Heuristic fallback for plain items
