...
/// Parse raw DNS server address into a typed DnsEndpoint.
/// Supported formats:
/// - udp (default 53): "8.8.8.8", "8.8.8.8:53", "udp://8.8.8.8:53", "udp://dns.google:53"
/// - tcp (default 53): "tcp://8.8.8.8:53", "tcp://dns.google:53"
/// - dot (default 853): "tcp-tls://8.8.8.8:853", "tls://8.8.8.8", "dot://dns.google"
/// - doh (default 443): "https://1.1.1.1/dns-query", "https://dns.google/dns-query"
...
fn parse_host_port(input: &str, default_port: u16) -> io::Result<(String, u16)> {
    ...
    // Check for IPv6 format: [2001:db8::1]:53 or [2001:db8::1]
    ...
    // Split on ':' from the right if not multiple colons (IPv6 without brackets)
...
impl DnsClient {
    ...
    async fn query_tcp(
        ...
        // TCP DNS requires 2-byte big-endian length prefix
    ...
    async fn query_doh(&self, url: &str, query: &[u8]) -> io::Result<Vec<IpAddr>> {
        ...
        let bytes = resp
            .bytes()
            .await
            .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("DoH body error: {e}")))?;
