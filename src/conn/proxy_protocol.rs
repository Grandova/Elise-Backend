...
/// A stream wrapper that can replay prefix bytes before reading from the underlying stream
...
/// Parse Proxy Protocol (v1 or v2) from an async stream with ConnectionMeta and trusted_proxies validation.
pub async fn read_proxy_protocol_with_meta<S: AsyncRead + AsyncWrite + Unpin>(
    ...
    // 1. Check v2 binary signature (12 bytes)
    ...
    // 2. Check v1 text signature: "PROXY "
    ...
    // Auto-detect mode: replay all read bytes without data loss
    Ok((meta, PrefixedStream::new(stream, Some(buf))))
}

/// Parse Proxy Protocol (v1 or v2) from any async stream with automatic prefix replay.
...
pub fn encode_proxy_protocol_v2(src: SocketAddr, dst: SocketAddr) -> Vec<u8> {
    ...
    match (src, dst) {
        (SocketAddr::V4(s), SocketAddr::V4(d)) => {
            ...
            buf.extend_from_slice(&12u16.to_be_bytes()); // length: 12 bytes
        ...
        (SocketAddr::V6(s), SocketAddr::V6(d)) => {
            ...
            buf.extend_from_slice(&36u16.to_be_bytes()); // length: 36 bytes
            buf.extend_from_slice(&s.ip().octets());
            buf.extend_from_slice(&d.ip().octets());
            buf.extend_from_slice(&s.port().to_be_bytes());
            buf.extend_from_slice(&d.port().to_be_bytes());
...
/// Parse Proxy Protocol from a UDP datagram (e.g. HAProxy or AWS NLB PROXY v2 over UDP)
...
mod tests {
    ...
    async fn test_meta_and_sticky_socks5_greeting_v1() {
        ...
        // Append SOCKS5 greeting directly into same buffer
    ...
    async fn test_trusted_proxies_filter() {
        ...
        // 1. Trusted peer should accept claimed client
        ...
        // 2. Untrusted peer in Auto mode should fall back to untrusted_peer
        ...
        // 3. Untrusted peer in Strict mode should error
