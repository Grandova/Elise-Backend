...
/// V2Board 客户端实现
/// 针对 https://github.com/wyx2685/v2board 及配套 v2node 规范：
/// - 请求固定携带 node_type=v2node&node_id={node_id}&token={token}
/// - 节点配置接口：GET /api/v2/server/config
...
impl V2BoardClient {
    ...
    /// 获取节点配置，严格对接 wyx2685/v2board 的 /api/v2/server/config
    ...
    /// 获取用户列表，支持 ETag 缓存与 node_type=v2node 查询参数
    pub async fn get_users(
        &self,
        node_id: u32,
    ) -> Result<Vec<User>, Box<dyn std::error::Error + Send + Sync>> {
        let url = format!(
            "{}/api/v1/server/UniProxy/user?node_type=v2node&node_id={}&token={}",
            self.base_url, node_id, self.token
        );
    ...
    pub async fn report_online_devices(
        ...
        let url = format!(
            "{}/api/v1/server/UniProxy/alive?node_type=v2node&node_id={}&token={}",
            self.base_url, node_id, self.token
        );
